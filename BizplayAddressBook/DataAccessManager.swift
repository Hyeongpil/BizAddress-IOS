...
final class DataAccessManager: NSObject {
    
    var bizplay_login_URL : String?
    var bizplay_apiKey : String?
    var bizplay_ptlID : String?
    var bizplay_apiId = LoginApiKey.BizplayLoginApiId
    
    func setURL(){
        let branch = Constant.branch
        if branch == 1{
            bizplay_login_URL = LoginApiKey.loginURL_develop
            bizplay_apiKey = LoginApiKey.loginApiKey_develop
            bizplay_ptlID = LoginApiKey.loginPtlID_develop
        }else{
            bizplay_login_URL = LoginApiKey.loginURL_release
            bizplay_apiKey = LoginApiKey.loginApiKey_release
            bizplay_ptlID = LoginApiKey.loginPtlID_release
        }
    }
  ...
  private func connectServer(apiKey: String,  apiId: String, suffixDic: [String:AnyObject],method: HTTPMethod,completion:(result:NSDictionary?, error: NSError?) -> Void) throws {
    ...
    var rootDic = CommonParamater(apiId: apiId, apiKey: apiKey).toDic()
    ...
    dataTask = urlSession.dataTaskWithRequest(url, completionHandler: { data, response, error in
      ...
      if error != nil {
        ...
        case 200: // 200 is OK
          ...
          if urlResponse.URL?.absoluteURL.absoluteString == ("\(self.bizplay_login_URL)/BizplayGate?") {
  ...
  private func urlForQuery(whatToQuery whatToQuery: [String:AnyObject],forMethod method: HTTPMethod) throws -> NSMutableURLRequest {
            ...
            guard let url = NSURL(string: "BizplayGate?JSONData=\(encodeString)", relativeToURL: try self.baseURL(self.bizplay_login_URL!)) else{
                throw URLError.InvalidURL
            }
